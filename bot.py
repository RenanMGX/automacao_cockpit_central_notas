...
class Execute:
    ...
    def start():
        ...
        sap_crd_param = execution.parameters.get('sap_crd')
        if not sap_crd_param:
            raise ValueError("Parâmetro 'sap_crd' não encontrado nos parâmetros da execução.")
        sap_crd = str(sap_crd_param)
        
        
        ExecuteAPP.start(
            sap_user=maestro.get_credential(label=sap_crd, key="user"),
            sap_password=maestro.get_credential(label=sap_crd, key="password"),
            sap_ambiente=maestro.get_credential(label=sap_crd, key="ambiente")
        )
